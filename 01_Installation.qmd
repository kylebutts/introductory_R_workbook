...
There are various parts of the Positron app and it can be overwhelming at first! 
...
knitr::include_graphics("figures/positron_user_interface.jpeg")
```
...
For example, if your working directory is set to your class folder, and you have a data file named `data.csv` stored inside a subfolder called `datasets`, you can load this file in R by using the relative path `datasets/data.csv` instead of the full path (e.g. `~/kylebutts/Documents/UARK\_4753/datasets/data.csv`). 
This is recommended for two reasons.
First, when collaborating with people because they do not have the same root directory as you (only I have `~/kylebutts/`). 
